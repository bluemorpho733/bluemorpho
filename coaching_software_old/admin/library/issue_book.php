...
<script type="text/javascript">
   function fill_detail(value){
    var book_id=document.getElementById('book_id_number').value;
			...
			cache: false,
              success: function(detail){
            // prompt("Copy to clipboard: Ctrl+C, Enter", detail);
			  if(detail!=0){
		  var res = detail.split("|?|");
	      $("#student_roll_no").val(value); 
		  $("#student_name").val(res[0]); 
		  $("#subject_course_name").val(res[1]); 
          $("#subject_code").val(res[2]);
          $("#course_code").val(res[6]);
          }else{
        ...
        <div class="box-body">
					...
					<div class="form-group" >
					  ...
					  <option value="">Select student</option>
					    <?php
							...
							$course_name1=get_course_name($course_code);
							$subject_code=$row22['subject_code'];
							$subject_name1=get_subject_name($subject_code);
							$student_father_name=$row22['student_father_name'];
							$student_father_contact_number=$row22['student_father_contact_number'];
							?>
							<option value="<?php echo $student_roll_no; ?>"><?php echo $student_name."[".$student_roll_no."]-"."[".$course_name1."-".$subject_name1."]-[".$student_father_name."-".$student_father_contact_number."]"; ?></option>
							<?php
							}
							?>
					  </select>
			...
			<input type="hidden" name="student_name" id="student_name" />
         	...
         	<div class="col-md-3 ">
				<div class="form-group" >
				    ...
				    <input type="text"  name="course_name" placeholder="Student Class"  id="subject_course_name" class="form-control" readonly/>
				</div>
			</div>
			<div class="col-md-3 ">				
			...
			<?php
			  ...
			  $copy_left=$no_of_copy-1;
				...
				<input type="hidden" name="course_id" id="course_code" />
				<input type="hidden" name="subject_id" id="subject_code"/>
				<div class="col-md-3 ">				
					 <div class="form-group" >
