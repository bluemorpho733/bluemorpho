...
<script>
function valid(id,date,emp_id,amount){   
var myval=confirm("Are you sure want to delete this record !!!!");
if(myval==true){
salary_delete(id,date,emp_id,amount);       
 }            
else  {      
return false;
 }       
  }    
function salary_delete(id,date,emp_id,amount){
$.ajax({
type: "POST",
url: access_link+"staff/emp_salary_list_particular_delete.php?id="+id+"&date="+date+"&amount="+amount+"&emp_id="+emp_id+"",
cache: false,
...
$emp_photo=$row2['emp_photo'];			
				...
				<?php 
				echo $salary_slip_pdf;
				?>
				<td><a target="_blank" href='<?php echo $pdf_path; ?>salary_slip/<?php echo $salary_slip_pdf; ?>?id=<?php echo $s_no1; ?>&emp_id=<?php echo $emp_id; ?>&date_diff=<?php echo $date_diff; ?>'><button type="button"  class="btn btn-default my_background_color"  >Print</button></a></td>
				<td><button type="button" class="btn btn-default my_background_color"  onclick="return valid('<?php echo $s_no1; ?>','<?php echo $employee_salary_generate_date; ?>','<?php echo $emp_id; ?>','<?php echo $final_salary; ?>');">
                Delete</button></td>
				</tr>
				<?php } } ?>
	    </tbody>
